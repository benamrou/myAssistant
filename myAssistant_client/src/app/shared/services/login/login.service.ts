...
export class LogginService{
    ...
    login(username: string, password: string): Observable<boolean> {
        ...
        return this._http.authentification(request, options).pipe(map(response => {
                ...
                if (token) {
                    ...
                    localStorage.setItem('myAssistantAuthToken', token);
                    localStorage.setItem('myAssistantUser', data.USERID);
                    localStorage.setItem('myAssistantSID', '');
                    localStorage.setItem('myAssistantLanguage', '');
    ...
    logout(): void {
        ...
        //console.log('***** Logout ****');
        this.token = null;
        localStorage.removeItem('myAssistantUser');
        localStorage.removeItem('myAssistantAuthToken');
        localStorage.removeItem('myAssistantSID');
        localStorage.removeItem('myAssistantLanguage');
    }
}
